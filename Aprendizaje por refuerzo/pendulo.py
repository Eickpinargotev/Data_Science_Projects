...
env = gym.make('Pendulum-v1', render_mode='rgb_array')  # Sin renderizado durante el entrenamiento

# Parámetros de aprendizaje
alpha = 0.05        # Tasa de aprendizaje ajustada a 0.1
gamma = 0.9       # Factor de descuento ajustado a 1.0
...
episodes = 20000        # Número de episodios aumentado a 15,000

bins = [20, 20, 20]  # Discretización de cada dimensión del estado ajustada a 30 bins
...
num_actions = len(np.linspace(-2, 2, 6))
space_actions=np.linspace(-2, 2, 6)
print(space_actions)
...
for episode in range(1, episodes + 1):
    ...
    while not (terminated or truncated):
        ...
        # Tomar acción
        next_state_continuous, reward, terminated, truncated, _ = env.step([space_actions[action]])
...
def probar_politica_aprendida(Q, env, bins_edges, num_steps=1000, delay=0.05):
    ...
    for _ in range(num_steps):
        ...
        state_continuous, reward, terminated, truncated, _ = env_render.step([space_actions[action]])
...
def simulate_learned_policy(Q, env, bins_edges, num_episodes=100, num_steps=1000):
    ...
    for episode in range(num_episodes):
        ...
        for step in range(num_steps):
            ...
            # Tomar acción
            state_continuous, reward, terminated, truncated, _ = env.step([space_actions[action]])
            total_reward += reward
...
plt.title('Progreso del Aprendizaje del Modelo en Pendulum')
plt.legend()
plt.grid(True)
plt.show()
